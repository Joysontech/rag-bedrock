...
The AIP-C01 exam is divided into five domains with the following weightings:

- Domain 1: Foundation Model Integration and Data Management covers **31%** of the exam
- Domain 2: GenAI Application Implementation and Integration covers **26%** of the exam
- Domain 3: AI Safety, Security and Governance covers **20%** of the exam
- Domain 4: Operational Excellence and Efficiency covers **12%** of the exam
- Domain 5: Testing, Validation and Troubleshooting covers **11%** of the exam

Domain 1 is the highest-weighted domain at 31%. Domain 2 is the second highest at 26%.

---

### Domain 1: Foundation Model Integration and Data Management — 31%

Domain 1 covers 31 percent of the AIP-C01 exam. This is the highest-weighted domain and focuses on how you select, configure, and integrate foundation models with data sources.
...
- pgvector cosine distance operator is the spaceship operator (<=>) . For normalised embeddings, cosine distance equals 1 minus cosine similarity.
...
Domain 2 covers 26 percent of the AIP-C01 exam. This domain covers the application layer: orchestration, prompt engineering, session management, and API integration.
...
- API Gateway and Lambda patterns: HTTP API vs REST API (HTTP API for JWT auth, REST API for API keys and WAF), payload format 2.0
...
**Critical exam gotchas:**
- The Bedrock console Prompt Management builder creates CHAT-type prompts (with system instructions and message turns), not TEXT-type. Your code must parse templateConfiguration.chat.messages not templateConfiguration.text.
- API Gateway HTTP API costs approximately 70% less than REST API and supports native JWT authorisation with Cognito. Use HTTP API for Lambda backends unless you specifically need REST API features (WAF, API keys, request transformation).
...
Domain 3 covers 20 percent of the AIP-C01 exam. This domain covers responsible AI, security controls, and compliance mechanisms.
...
- Guardrail intervention modes: hard block (stop_reason equals guardrail_intervened) vs message substitution (HTTP 200 with replaced content)
- Contextual grounding threshold: 0 to 1, where higher means stricter checking. 0.75 is a common production value.
- Prompt injection attacks: indirect prompt injection via document content, direct prompt injection via user input. Prompt Attack filter set to High detects both types.
...
Domain 4 covers 12 percent of the AIP-C01 exam. This domain covers cost optimisation, performance, scaling, and model lifecycle management.
...
- Bedrock model cost comparison: Haiku (cheapest) then Sonnet then Opus. Embedding models cheaper than generation models.
...
Domain 5 covers 11 percent of the AIP-C01 exam. This domain covers evaluation frameworks, quality measurement, and debugging AI systems.
...
- RAG evaluation metrics: Faithfulness (is the answer grounded in the retrieved context, detects hallucination), Correctness (is the answer factually accurate vs a reference answer), Completeness (does the answer fully address the question), Relevance (is the answer on-topic and directly responsive)
...
| Titan Multimodal Embeddings | Image and text | 384/1024 dims, image similarity |
...
Answer: The model ARN uses a cross-region inference profile ID with the eu. prefix. RetrieveAndGenerate only accepts direct foundation model ARNs. Remove the regional prefix.
...
1. Build it, do not just read it: spin up a Bedrock Knowledge Base, a Guardrail, and a Prompt Management template in the console. The exam tests operational knowledge, not documentation recall.
...
7. Cost model: Haiku is cheaper than Sonnet which is cheaper than Opus for generation. On-demand is better than provisioned for variable workloads. Use scale-to-zero Aurora for dev builds.
...
- Udemy course: Ultimate AWS Certified Generative AI Developer Professional covers hands-on labs for all five domains
